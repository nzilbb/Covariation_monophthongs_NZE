...
server <- function(input, output, session) {
  ...
  output$distPlot1 <- renderPlot({
    ...
    # Comp.1_distinct <- sound_change_plot_data %>%
    #   distinct(Comp.1) %>%
    #   arrange(Comp.1)
    # 
    # sound_change_plot_data %>%
    #   filter(Comp.1 == Comp.1_distinct[findInterval(input$PC1, Comp.1_distinct$Comp.1),]) %>%
    #   #set general aesthetics
    #   ggplot(aes(x = F2_PC1, y = F1_PC1, colour = Vowel, group = Vowel, label = Vowel, size = PC1_loadings_abs)) +
    #   geom_text(aes(fontface = 2), show.legend = FALSE) +
    #   #label the axes
    #   xlab("F2 (normalised)") +
    #   ylab("F1 (normalised)") +
  ...
  output$distPlot2 <- renderPlot({
    ...
    # mod_pred_PC_plot_data %>%
    #   filter(Comp.2 == as.numeric(Comp.2_distinct[findInterval(input$PC2, Comp.2_distinct$Comp.2),])) %>%
    #   #set general aesthetics
    #   ggplot(aes(x = F2_PC2, y = F1_PC2, colour = Vowel, group = Vowel, label = Vowel, size = PC2_loadings_abs)) +
    #   geom_text(aes(fontface = 2), show.legend = FALSE) +
    #   #label the axes
    #   xlab("F2 (normalised)") +
    #   ylab("F1 (normalised)") +
  ...
  #   mod_pred_PC_plot_data %>%
  #     filter(Comp.3 == Comp.3_distinct[findInterval(input$PC3, Comp.3_distinct$Comp.3),]) %>%
  #     #set general aesthetics
  #     ggplot(aes(x = F2_PC3, y = F1_PC3, colour = Vowel, group = Vowel, label = Vowel, size = PC3_loadings_abs)) +
  #     geom_text(aes(fontface = 2), show.legend = FALSE) +
  #     #label the axes
  #     xlab("F2 (normalised)") +
  #     ylab("F1 (normalised)") +
