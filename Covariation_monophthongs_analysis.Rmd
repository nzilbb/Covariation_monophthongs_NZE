...
vowels_all <- readRDS("Data/ONZE_vowels_filtered_anon.rds")
 ...
 # %>%
 #  mutate(Speaker = gsub(x = Speaker, pattern = "Darfield", "CRS"),
 #         Transcript = gsub(x = Transcript, pattern = "Darfield", "CRS"),
 #         Corpus = gsub(x = Corpus, pattern = "Darfield", "CRS"))
...
Darfield - Canterbury Regional Survey<br>
...
demographics1 <- vowels_all %>%
  ...
  summarise(`N Tokens` = n(), #n tokens
            `%` = `N Tokens`/nrow(vowels_all)*100, #n tokens as a percent
...
demographics %>%
  kable(digits = 1, align = c("l", "c", "c", "c", "c", "c", "c")) %>% #make table and round decimals to 1
...
In order to statistically assess whether the PC scores are reliably predicted by any of the social variables, we will run a series of GAMs, predicting the PC scores in each of the 3 PCs by each of the social variables. We will also provide visualisations of the results (reproducing **Figure XX**).
